...
HERE = os.path.dirname(os.path.abspath(__file__))      # the mk/ folder this file lives in
EMB  = os.path.join(HERE, "npy", "ref_embeddings.npy") # 01's embeddings
META = os.path.join(HERE, "ref_metadata.csv")          # 01's labels (same order as EMB)
DEFS_CSV = os.path.join(HERE, "definitions.csv")       # code -> definition text
RESULTS_DIR = os.path.join(HERE, "results")            # where we write outputs
